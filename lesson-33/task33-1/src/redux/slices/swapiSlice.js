...
export const addSwapiItem = text => async dispatch => {
	...
	try {
		...
		const mockapiResponse = await fetch(API.URL_MOCKAPI_SWAPI);
		...
		const url = existingItem
			? `${API.URL_MOCKAPI_SWAPI}/${existingItem.id}`
			: API.URL_MOCKAPI_SWAPI;
...
export const getSwapiItem = () => async dispatch => {
	try {
		const responseMockapi = await fetch(API.URL_MOCKAPI_SWAPI);
...
export const clearSwapiItems = () => async dispatch => {
	...
	try {
		...
		for (const item of mockapiData) {
			await fetch(`${API.URL_MOCKAPI_SWAPI}/${item.id}`, {
				method: 'DELETE',
			});
		}
